  ...
  switch (actionType) {
    case 'set_field':
      return (
        <div className="space-y-1.5">
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'مسار الحقل' : 'Field Path'}</Label>
            <Input
          ...
          <div className="flex gap-2">
            <div className="flex-1">
              <Label className="text-[10px] text-gray-400">{isAr ? 'العملية' : 'Operation'}</Label>
              <select
    ...
    case 'notify':
      return (
        <div className="space-y-1.5">
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'المستلم' : 'To (userId / expression)'}</Label>
            <Input
              value={config.to || ''}
          ...
          <div className="flex gap-2">
            <div className="flex-1">
              <Label className="text-[10px] text-gray-400">{isAr ? 'القالب' : 'Template'}</Label>
              <Input
            ...
            <div className="flex-1">
              <Label className="text-[10px] text-gray-400">{isAr ? 'القناة' : 'Channel'}</Label>
              <select
    ...
    case 'send_email':
      return (
        <div className="space-y-1.5">
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'المستلم' : 'To'}</Label>
            <Input
              value={config.to || ''}
              onChange={(e) => patch({ to: e.target.value })}
          ...
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'المحتوى' : 'Body Template'}</Label>
            <textarea
              value={config.body || ''}
              onChange={(e) => patch({ body: e.target.value })}
              placeholder="Hello {{assignee}}, ..."
              rows={2}
              className="w-full rounded border border-gray-300 bg-white px-2 py-1 text-xs mt-0.5 focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none"
            />
          </div>
        </div>
      );
    ...
    case 'assign':
      return (
        <div className="space-y-1.5">
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'الاستراتيجية' : 'Strategy'}</Label>
            <select
          ...
          {config.strategy === 'specific' || !config.strategy ? (
            <div>
              <Label className="text-[10px] text-gray-400">{isAr ? 'معرف المستخدم' : 'User ID'}</Label>
              <Input
                value={config.userId || ''}
            ...
            <div>
              <Label className="text-[10px] text-gray-400">{isAr ? 'معرف المجموعة' : 'Group ID'}</Label>
              <Input
    ...
    case 'create_task':
      return (
        <div className="space-y-1.5">
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'العنوان' : 'Title'}</Label>
            <Input
              value={config.title || ''}
          ...
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'الوصف' : 'Description'}</Label>
            <textarea
          ...
          <div className="flex gap-2">
            <div className="flex-1">
              <Label className="text-[10px] text-gray-400">{isAr ? 'المسؤول' : 'Assignee'}</Label>
              <Input
    ...
    case 'call_webhook':
      return (
        <div className="space-y-1.5">
          ...
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'الطريقة' : 'Method'}</Label>
            <select
          ...
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'الجسم' : 'Body Template (JSON)'}</Label>
            <textarea
              value={config.bodyTemplate || ''}
              onChange={(e) => patch({ bodyTemplate: e.target.value })}
              placeholder='{"ticketId": "{{entity.id}}"}'
              rows={2}
              className="w-full rounded border border-gray-300 bg-white px-2 py-1 text-xs mt-0.5 font-mono focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none"
            />
          </div>
        </div>
      );
    ...
    case 'call_api':
      return (
        <div className="space-y-1.5">
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'النقطة النهائية' : 'Endpoint'}</Label>
            <Input
          ...
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'البيانات (JSON)' : 'Payload (JSON)'}</Label>
            <textarea
    ...
    case 'escalate':
      return (
        <div className="space-y-1.5">
          <div className="flex gap-2">
            <div className="flex-1">
              <Label className="text-[10px] text-gray-400">{isAr ? 'المستوى' : 'Level'}</Label>
              <Input
                type="number"
                min={1}
                max={5}
            ...
            <div className="flex-1">
              <Label className="text-[10px] text-gray-400">{isAr ? 'الدور المستهدف' : 'Target Role'}</Label>
              <Input
          ...
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'قالب الإشعار' : 'Notify Template'}</Label>
            <Input
    ...
    case 'update_entity':
      return (
        <div>
          <Label className="text-[10px] text-gray-400">{isAr ? 'التحديثات (JSON)' : 'Updates (JSON)'}</Label>
          <textarea
    ...
    case 'log_activity':
      return (
        <div>
          <Label className="text-[10px] text-gray-400">{isAr ? 'الرسالة' : 'Message Template'}</Label>
          <Input
    ...
    case 'add_comment':
      return (
        <div className="space-y-1.5">
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'التعليق' : 'Comment Message'}</Label>
            <textarea
              value={config.message || ''}
              onChange={(e) => patch({ message: e.target.value })}
          ...
          <label className="flex items-center gap-2">
            ...
            />
            <span className="text-[10px] text-gray-400">{isAr ? 'تعليق داخلي' : 'Internal comment'}</span>
          </label>
        </div>
      );
    ...
    case 'run_script':
      return (
        <div>
          <Label className="text-[10px] text-gray-400">{isAr ? 'السكربت' : 'Script Body'}</Label>
          <textarea
    ...
    case 'external_task':
      return (
        <div>
          <Label className="text-[10px] text-gray-400">{isAr ? 'الموضوع (Topic)' : 'Topic'}</Label>
          <Input
            value={config.topic || ''}
    ...
    case 'custom':
      return (
        <div className="space-y-1.5">
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'اسم الدالة' : 'Function Name'}</Label>
            <Input
          ...
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'معلمات (JSON)' : 'Params (JSON)'}</Label>
            <textarea
