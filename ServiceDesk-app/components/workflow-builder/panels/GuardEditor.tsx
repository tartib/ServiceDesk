  ...
  return (
    <div className="space-y-2">
      ...
      {guards.map((guard, idx) => (
        <div
          key={guard.guardId}
          className="border border-gray-200 rounded-lg p-2.5 space-y-2 bg-gray-50/50"
        >
          {/* Header: type + delete */}
          <div className="flex items-center gap-2">
            <select
              ...
              className="flex-1 h-7 rounded border border-gray-300 bg-white px-2 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500"
            >
              {GUARD_TYPES.map((t) => (
          ...
          {guard.type === 'role' && (
            <div>
              <Label className="text-[10px] text-gray-400">{isAr ? 'الأدوار (فاصلة)' : 'Roles (comma-separated)'}</Label>
              <Input
          ...
          {guard.type === 'expression' && (
            <div>
              <Label className="text-[10px] text-gray-400">{isAr ? 'التعبير' : 'Expression'}</Label>
              <textarea
          ...
          {guard.type === 'time_window' && (
            <div className="flex gap-2">
              <div className="flex-1">
                <Label className="text-[10px] text-gray-400">{isAr ? 'بعد (ساعات)' : 'After (hrs)'}</Label>
                <Input
                  type="number"
                  min={0}
              ...
              <div className="flex-1">
                <Label className="text-[10px] text-gray-400">{isAr ? 'قبل (ساعات)' : 'Before (hrs)'}</Label>
                <Input
                  type="number"
                  min={0}
          ...
          {/* Error message */}
          <div>
            <Label className="text-[10px] text-gray-400">{isAr ? 'رسالة الخطأ' : 'Error Message'}</Label>
            <Input
