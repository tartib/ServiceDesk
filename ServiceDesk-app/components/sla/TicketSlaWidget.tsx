...
const statusConfig: Record<string, { icon: typeof Clock; color: string; bg: string; label: string }> = {
  ...
  paused: { icon: PauseCircle, color: 'text-yellow-600', bg: 'bg-yellow-50', label: 'Paused' },
  met: { icon: CheckCircle, color: 'text-green-600', bg: 'bg-green-50', label: 'Met' },
  breached: { icon: XCircle, color: 'text-red-600', bg: 'bg-red-50', label: 'Breached' },
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="w-full bg-gray-200 rounded-full h-1.5">
          <div
            className={`h-1.5 rounded-full transition-all ${
              metric.breached ? 'bg-red-500' : isUrgent ? 'bg-yellow-500' : 'bg-blue-500'
            }`}
        ...
        <div className="flex items-center justify-between mt-1">
          ...
          {(metric.status === 'running' || metric.status === 'paused') && (
            <span className={`text-[10px] font-medium ${isUrgent ? 'text-red-600' : 'text-gray-500'}`}>
...
export default function TicketSlaWidget({ ticketId }: TicketSlaWidgetProps) {
  ...
  return (
    <div className={`rounded-xl border p-4 ${hasBreaches ? 'border-red-200 bg-red-50/30' : 'border-gray-200 bg-white'}`}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-900">SLA</span>
        </div>
