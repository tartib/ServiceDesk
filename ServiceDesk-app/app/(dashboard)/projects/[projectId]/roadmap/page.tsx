...
export default function RoadmapPage() {
 ...
 <span className="text-sm font-semibold text-foreground">{t('roadmap.columns.workItems')}</span>
 </div>
 </div>
 ...
 <div key={epic._id}>
 {/* Epic Row */}
 <div className={`flex border-b border-border ${epicIndex % 2 === 0 ? 'bg-background' : 'bg-muted/50/30'}`} style={{ minHeight: '80px' }}>
 {/* Sticky left cell */}
 <div className={`sticky left-0 z-20 w-96 shrink-0 border-r border-border px-3 sm:px-4 py-3 hover:bg-brand-surface transition-colors cursor-pointer ${epicIndex % 2 === 0 ? 'bg-background' : 'bg-muted/50/30'}`}>
 <div className="flex items-center gap-2">
 <GripVertical className="h-4 w-4 text-muted-foreground cursor-grab hover:text-muted-foreground" />
 <button onClick={() => toggleItem(epic._id)} className="p-0.5 text-muted-foreground hover:text-foreground hover:bg-muted rounded" aria-expanded={expandedItems.has(epic._id)}>
 ...
 <button onClick={() => { setSelectedTask(epic); setShowTaskDetail(true); }} className="text-brand text-xs sm:text-sm font-mono hover:underline">
 {epic.key}
 </button>
 <span className="text-foreground text-sm font-medium flex-1 break-words">{epic.title || epic.summary}</span>
 </div>
 <div className="ms-16 mt-2 flex items-center gap-2">
 <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
 <div className={`h-full rounded-full transition-all duration-500 ${getEpicProgress(epic._id) === 100 ? 'bg-success' : getEpicProgress(epic._id) >= 50 ? 'bg-brand' : getEpicProgress(epic._id) > 0 ? 'bg-warning' : 'bg-muted'}`} style={{ width: `${getEpicProgress(epic._id)}%` }}></div>
 </div>
 <span className={`text-xs font-medium ${getEpicProgress(epic._id) === 100 ? 'text-success' : getEpicProgress(epic._id) >= 50 ? 'text-brand' : getEpicProgress(epic._id) > 0 ? 'text-warning' : 'text-muted-foreground'}`}>{getEpicProgress(epic._id)}%</span>
 </div>
 </div>
 {/* Timeline cells */}
 ...
 {/* Epic Bar */}
 <div 
 className={`absolute h-7 rounded-md shadow-sm cursor-grab active:cursor-grabbing transition-all group/bar ${hoveredTask === epic._id ? 'shadow-lg scale-[1.02] z-20' : ''} ${selectedTask?._id === epic._id ? 'ring-2 ring-info/40 shadow-lg z-20' : ''} ${isDragging && draggedItem === epic._id ? 'opacity-70 scale-105 z-30' : ''}`}
 style={{
 ...
 <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 bg-background rounded-full p-0.5 shadow-sm"><ArrowLeft className="h-3 w-3 text-info" /></div>
 )}
 {getTaskBarStyle(epic, timelineColumns.length).showRightArrow && (
 <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 bg-background rounded-full p-0.5 shadow-sm"><ArrowRight className="h-3 w-3 text-info" /></div>
 )}
 <div className="absolute inset-y-0 left-0 bg-background/30 rounded-l-md transition-all duration-500" style={{ width: `${getEpicProgress(epic._id)}%` }} />
 ...
 <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-foreground text-background text-xs px-3 py-2 rounded-lg shadow-xl whitespace-nowrap z-50 pointer-events-none">
 ...
 <div key={task._id} className={`flex border-b border-border ${taskIndex % 2 === 0 ? 'bg-background' : 'bg-muted/50/20'}`} style={{ minHeight: '56px' }}>
 {/* Sticky left cell */}
 <div className={`sticky left-0 z-20 w-96 shrink-0 border-r border-border ps-10 pe-3 sm:pe-4 py-2.5 hover:bg-brand-surface transition-colors ${taskIndex % 2 === 0 ? 'bg-background' : 'bg-muted/50/20'}`}>
 <div className="flex items-center gap-2">
 <GripVertical className="h-3.5 w-3.5 text-muted-foreground cursor-grab" />
 <span className={`${getTypeColor(task.type)} text-sm`}>{getTypeIcon(task.type)}</span>
 <button onClick={() => { setSelectedTask(task); setShowTaskDetail(true); }} className="text-brand text-xs font-mono hover:underline">{task.key}</button>
 <span className="text-foreground text-sm flex-1 break-words">{task.title || task.summary}</span>
 </div>
 <div className="ms-12 mt-1 flex items-center gap-2">
 <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
 <div className={`h-full rounded-full transition-all duration-500 ${getTaskProgress(task) === 100 ? 'bg-success' : getTaskProgress(task) >= 50 ? 'bg-brand' : getTaskProgress(task) > 0 ? 'bg-warning' : 'bg-muted'}`} style={{ width: `${getTaskProgress(task)}%` }}></div>
 </div>
 <span className={`text-xs font-medium ${getTaskProgress(task) === 100 ? 'text-success' : getTaskProgress(task) >= 50 ? 'text-brand' : getTaskProgress(task) > 0 ? 'text-warning' : 'text-muted-foreground'}`}>{getTaskProgress(task)}%</span>
 </div>
 </div>
 {/* Timeline cells */}
 <div className="relative flex flex-1">
 ...
 {/* Task Bar */}
 <div 
 className={`absolute h-5 rounded cursor-grab active:cursor-grabbing transition-all ${hoveredTask === task._id ? 'shadow-lg scale-[1.02] z-20' : ''} ${selectedTask?._id === task._id ? 'ring-2 ring-emerald/40 shadow-lg z-20' : ''}`}
 style={{
 ...
 {getTaskBarStyle(task, timelineColumns.length).showLeftArrow && <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 bg-background rounded-full p-0.5 shadow-sm"><ArrowLeft className="h-2.5 w-2.5 text-brand" /></div>}
 {getTaskBarStyle(task, timelineColumns.length).showRightArrow && <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 bg-background rounded-full p-0.5 shadow-sm"><ArrowRight className="h-2.5 w-2.5 text-brand" /></div>}
 ...
 <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-foreground text-background text-xs px-2 py-1.5 rounded-lg shadow-xl whitespace-nowrap z-50 pointer-events-none">
 ...
 <div key={task._id} className={`flex border-b border-border ${(groupedTasks.epics.length + idx) % 2 === 0 ? 'bg-background' : 'bg-muted/50/30'}`} style={{ minHeight: '56px' }}>
 {/* Sticky left cell */}
 <div className={`sticky left-0 z-20 w-96 shrink-0 border-r border-border px-3 sm:px-4 py-3 hover:bg-brand-surface transition-colors ${(groupedTasks.epics.length + idx) % 2 === 0 ? 'bg-background' : 'bg-muted/50/30'}`}>
 <div className="flex items-center gap-2">
 <GripVertical className="h-4 w-4 text-muted-foreground cursor-grab" />
 <span className={`${getTypeColor(task.type)} text-lg`}>{getTypeIcon(task.type)}</span>
 <button onClick={() => { setSelectedTask(task); setShowTaskDetail(true); }} className="text-brand text-xs sm:text-sm font-mono hover:underline">{task.key}</button>
 <span className="text-foreground text-sm font-medium flex-1 break-words">{task.title || task.summary}</span>
 </div>
 ...
 {/* Task Bar */}
 <div 
 className={`absolute h-5 rounded cursor-pointer transition-all ${hoveredTask === task._id ? 'shadow-lg scale-[1.02] z-20' : ''} ${selectedTask?._id === task._id ? 'ring-2 ring-brand-border shadow-lg z-20' : ''}`}
 style={{
 ...getTaskBarStyle(task, timelineColumns.length),
 top: '50%', transform: 'translateY(-50%)',
 background: task.type === 'bug' ? 'linear-gradient(135deg, #ef4444 0%, #dc2626 100%)' : task.type === 'story' ? 'linear-gradient(135deg, #22c55e 0%, #16a34a 100%)' : 'linear-gradient(135deg, #ffffff 0%, #2563eb 100%)'
 ...
 {/* Panel Container */}
 <div 
 className="relative w-full max-w-lg bg-foreground text-background shadow-2xl animate-in slide-in-from-right h-full overflow-hidden"
 role="dialog"
 ...
 {/* Issue Key */}
 <span className="text-brand font-mono text-sm hover:underline cursor-pointer">
 {selectedTask.key}
 </span>
 </div>
 ...
 <button 
 className="p-2 text-muted-foreground hover:text-background hover:bg-foreground/80 rounded transition-colors"
 title="Watch"
 ...
 <button 
 className="px-2 py-1 text-xs bg-brand text-brand-foreground rounded flex items-center gap-1 hover:bg-brand-strong transition-colors"
 title="Watchers"
 >
 ...
 className="p-2 text-muted-foreground hover:text-background hover:bg-foreground/80 rounded transition-colors ms-2"
 ...
 <h1 
 className="text-xl font-semibold text-background flex-1 cursor-text hover:bg-foreground/90 rounded px-1 -mx-1 transition-colors"
 contentEditable
 ...
 className="px-3 py-1.5 bg-success text-success-foreground text-sm font-medium rounded hover:bg-success/80 transition-colors flex items-center gap-1"
 aria-haspopup="listbox"
 >
 ...
 <button className="p-1.5 text-muted-foreground hover:text-background hover:bg-foreground/80 rounded" title="Automation">
 <Zap className="h-4 w-4" />
 </button>
 <button className="p-1.5 text-muted-foreground hover:text-background hover:bg-foreground/80 rounded" title="Settings">
 <Settings className="h-4 w-4" />
 </button>
 <button className="p-1.5 text-muted-foreground hover:text-background hover:bg-foreground/80 rounded" title="Add">
 <Plus className="h-4 w-4" />
 </button>
 </div>
 </div>
 ...
 <div 
 className="text-muted-foreground text-sm cursor-text hover:bg-foreground/90 rounded p-2 -m-2 min-h-[60px] transition-colors"
 contentEditable
 suppressContentEditableWarning
 onBlur={async (e) => {
 ...
 >
 {selectedTask.description || <span className="text-muted-foreground italic">Add a description...</span>}
 </div>
 </div>
 ...
 <button className="text-brand text-sm hover:text-brand flex items-center gap-1">
 <Plus className="h-4 w-4" />
 Add child work item
 </button>
 </div>
 ...
 <button className="p-1 text-muted-foreground hover:text-white" title="Configure fields">
 <Settings className="h-4 w-4" />
 </button>
 </div>
 ...
 <button className="flex items-center gap-2 w-full hover:bg-foreground/90 rounded p-1 -m-1 transition-colors">
 {selectedTask.assignee ? (
 <>
 <div className="w-6 h-6 rounded-full bg-warning flex items-center justify-center text-white text-xs font-medium">
 ...
 <>
 <div className="w-6 h-6 rounded-full bg-muted-foreground/70 flex items-center justify-center">
 ...
 <button className="text-brand text-sm hover:text-brand">
 Add labels
 </button>
 </div>
 ...
 {/* Sprint */}
 <div className="mb-4">
 <label className="text-xs text-muted-foreground block mb-1">Sprint</label>
 <button className="text-muted-foreground text-sm hover:text-white">
 None
 </button>
 </div>
 ...
 {/* Story Points */}
 <div className="mb-4">
 <label className="text-xs text-muted-foreground block mb-1">Story Points</label>
 <input 
 type="number"
 min="0"
 max="100"
 ...
 className="bg-transparent text-white text-sm w-16 border-b border-border focus:border-brand focus:outline-none"
 />
 </div>
 ...
 className="bg-foreground/90 text-background text-sm rounded p-1 border border-border cursor-pointer"
 >
 ...
 {/* Start Date */}
 <div className="mb-4">
 <label className="text-xs text-muted-foreground block mb-1">{t('roadmap.dates.startDate')}</label>
 <input 
 type="date" 
 ...
 {/* Due Date */}
 <div className="mb-4">
 <label className="text-xs text-muted-foreground block mb-1">{t('roadmap.dates.dueDate')}</label>
 <input 
 type="date" 
 ...
 <h3 className="text-xs text-muted-foreground mb-2">Development</h3>
 <button className="text-brand text-sm hover:text-brand flex items-center gap-1">
 <Plus className="h-4 w-4" />
 Create branch
 </button>
