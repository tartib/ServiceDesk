...
const STATUS_BADGE: Record<PIRStatus, string> = {
  ...
  [PIRStatus.IN_REVIEW]: 'bg-blue-100 text-blue-700',
  [PIRStatus.COMPLETED]: 'bg-green-100 text-green-700',
};
    ...
    return (
      <DashboardLayout>
        <div className="text-center py-16">
          ...
          <Link href="/pirs" className="mt-4 inline-block text-primary hover:underline">← Back to PIRs</Link>
        </div>
      </DashboardLayout>
    );
  }
            ...
            <div>
              ...
              {pirData.incident_title && (
                <p className="text-sm text-muted-foreground mt-1 ml-12">
                  Re:{' '}
                  <Link href={`/incidents/${pirData.incident_id}`} className="text-primary hover:underline">
          ...
          <div className="flex items-center gap-2">
            ...
            {pirData.status === PIRStatus.IN_REVIEW && (
              <button
                ...
                className="px-4 py-2 bg-success text-white rounded-lg hover:bg-success/90 text-sm font-medium disabled:opacity-50"
              >
              ...
              <div className="space-y-5">
                ...
                <Card className="p-6">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="font-semibold text-foreground flex items-center gap-2">
                      <Target className="w-4 h-4 text-purple-500" /> Impact Summary
                    </h2>
                    {isEditable && !editing && (
                      <button onClick={() => { setEditing(true); setForm({ impact_summary: pirData.impact_summary }); }} className="text-xs text-primary hover:underline">Edit</button>
                    )}
                  </div>
                  {editing ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-green-50 dark:bg-green-900/10 rounded-xl border border-green-200 dark:border-green-800 p-5">
                    ...
                    <p className="text-green-900 dark:text-green-200 text-sm whitespace-pre-wrap">
                      {pirData.what_went_well || <span className="text-green-400/70 italic">Not filled in</span>}
                    </p>
                  </div>
                  <div className="bg-red-50 dark:bg-red-900/10 rounded-xl border border-red-200 dark:border-red-800 p-5">
                    <h3 className="font-semibold text-red-800 dark:text-red-300 flex items-center gap-2 mb-3 text-sm">
                      <ThumbsDown className="w-4 h-4" /> What Went Wrong
                    </h3>
                    <p className="text-red-900 dark:text-red-200 text-sm whitespace-pre-wrap">
                      {pirData.what_went_wrong || <span className="text-red-400/70 italic">Not filled in</span>}
                    </p>
                  </div>
                </div>
                ...
                <Card className="p-6">
                  <h2 className="font-semibold text-foreground flex items-center gap-2 mb-4">
                    <Clock className="w-4 h-4 text-blue-500" /> Timeline of Events
                  </h2>
                  <p className="text-muted-foreground text-sm whitespace-pre-wrap">
              ...
              <div className="space-y-5">
                <Card className="p-6">
                  <h2 className="font-semibold text-foreground flex items-center gap-2 mb-5">
                    <Lightbulb className="w-4 h-4 text-yellow-500" /> Root Cause Analysis
                  </h2>
                  <div className="space-y-5">
                    <div>
            ...
            <Card className="p-5">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-primary text-sm group-hover:underline truncate">{pirData.incident_id}</p>
