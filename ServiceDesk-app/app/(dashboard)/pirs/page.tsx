            ...
            {Object.entries(STATUS_CONFIG).map(([status, cfg]) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
                  statusFilter === status ? 'bg-primary text-primary-foreground ring-2 ring-offset-1 ring-primary/40' : 'bg-muted text-muted-foreground hover:bg-accent'
                }`}
              >
                {cfg.label}
              </button>
                ...
                {filtered.map((pir) => {
                  ...
                  return (
                    <TableRow key={pir._id}>
                      <TableCell>
                        <Link href={`/pirs/${pir._id}`} className="font-medium text-purple-600 hover:text-purple-700 text-sm">
                          {pir.pir_id}
                        </Link>
                      </TableCell>
                      <TableCell>
                        <div>
                          <Link href={`/incidents/${pir.incident_id}`} className="text-sm text-primary hover:underline font-medium">
                            {pir.incident_id}
                          </Link>
                          {pir.incident_title && (
