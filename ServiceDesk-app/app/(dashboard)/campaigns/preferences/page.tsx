...
export default function PreferencesPage() {
  ...
  const channelToggles: { key: ToggleField; label: string; icon: React.ReactNode }[] = [
    { key: 'emailEnabled', label: 'Email Notifications', icon: <Mail className="h-5 w-5 text-blue-500" /> },
    { key: 'smsEnabled', label: 'SMS Notifications', icon: <MessageSquare className="h-5 w-5 text-green-500" /> },
    { key: 'pushEnabled', label: 'Push Notifications', icon: <Bell className="h-5 w-5 text-purple-500" /> },
  ];
                ...
                <div key={ch.key} className="flex items-center justify-between py-2">
                  ...
                  <button
                    ...
                    className={`w-10 h-5 rounded-full transition-colors relative ${
                      preferences?.[ch.key] !== false ? 'bg-blue-600' : 'bg-gray-300'
                    }`}
                  >
                    <span
              ...
              {categoryToggles.map((cat) => (
                <div key={cat.key} className="flex items-center justify-between py-2">
                  ...
                  <button
                    ...
                    disabled={updateMutation.isPending}
                    className={`w-10 h-5 rounded-full transition-colors relative ${
                      preferences?.[cat.key] !== false ? 'bg-blue-600' : 'bg-gray-300'
                    }`}
                  >
                    <span
                      className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${
