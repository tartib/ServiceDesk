...
export const createTask = async (req: PMAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (assignee && assignee.toString() !== userId) {
      notifyUser(assignee, {
        projectId,
        organizationId,
        type: 'assignment',
        title: `Assigned to ${task.key}`,
        message: `You were assigned to "${title}"`,
        actionUrl: `/projects/${projectId}/board?selectedIssue=${task._id}`,
      }).catch(() => {});
    }
  ...
  try {
    ...
    if (isPmPostgres()) {
      ...
      const result = await taskRepo.searchTasks(
        { projectId, statusId: status, type, assignee, sprintId, epicId, parentId, priority, q: search, sort: 'createdAt', sortDir: 'desc' },
        pageNum,
        limitNum,
      );
      res.status(200).json({
...
export const getTask = async (req: PMAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    let task: any;
    let project: any;
    let subtaskProgress: any;

    if (usePg) {
      const taskRepo = getPmRepos().task as PgTaskRepository;
      const projRepo = getPmRepos().project as PgProjectRepository;
      task = await taskRepo.findByKey(taskId) || await taskRepo.findById(taskId);
      if (!task) {
        res.status(404).json({ success: false, error: 'Task not found' } as ApiResponse);
        return;
      }
      project = await projRepo.findById(task.projectId);
      subtaskProgress = await taskRepo.getSubtaskProgress(task._id || task.id);
    } else {
    ...
    // Get available transitions (uses workflow service, Mongo-only for now)
    const transitions = usePg ? [] : await workflowService.getAvailableTransitions(taskId);

    // Check if current user is watching this task
    const watchers = task.watchers || [];
    const isWatching = watchers.some((w: any) => (w._id?.toString() || w.toString()) === userId) || false;
...
export const updateTask = async (req: PMAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Record activity and send notifications for each meaningful change
    ...
    for (const change of changes) {
      ...
      // Notify on assignee change — notify new assignee
      if (change.field === 'assignee' && change.newValue) {
        const newAssigneeId = change.newValue.toString();
        if (newAssigneeId !== userId) {
          notifyUser(newAssigneeId, {
            ...
            actionUrl: `/projects/${task.projectId}/board?selectedIssue=${task._id}`,
          }).catch(() => {});
        }
      }
      ...
      if (change.field === 'status') {
        const newStatus = (change.newValue as any)?.name || change.newValue?.toString() || '';
        const recipients: string[] = [];
        if (task.assignee) recipients.push(task.assignee.toString());
        if (task.reporter) recipients.push(task.reporter.toString());
        notifyUsers(recipients, {
          projectId: task.projectId,
          organizationId: task.organizationId,
          type: 'task',
          title: `${task.key} status updated`,
          message: `"${task.title}" was moved to ${newStatus}`,
          actionUrl: `/projects/${task.projectId}/board?selectedIssue=${task._id}`,
        }, userId).catch(() => {});
      }
    }
