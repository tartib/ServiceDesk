...
import { notifyProjectMembers } from './notificationHelper';
import { getPmRepos, isPmPostgres } from '../infrastructure/repositories';
import { PgSprintRepository } from '../infrastructure/repositories/PgSprintRepository';
...
export const createSprint = async (req: PMAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (usePg) {
      const projRepo = getPmRepos().project as PgProjectRepository;
      const project = await projRepo.findById(projectId);
      if (!project) {
        res.status(404).json({ success: false, error: 'Project not found' } as ApiResponse);
        return;
      }
      organizationId = project.organizationId || project.organization_id || req.user?.organizationId;
    } else {
      ...
      organizationId = project.organizationId?.toString() || req.user?.organizationId;
    }
    ...
    // ── PostgreSQL path ──
    if (usePg) {
      const sprintRepo = getPmRepos().sprint as PgSprintRepository;
      const nextNumber = await sprintRepo.getNextNumber(projectId);
      const sprint = await sprintRepo.create({
        ...
        startDate,
        endDate,
        capacityPlanned: capacity?.planned || 0,
        capacityCommitted: capacity?.committed || 0,
        createdBy: userId,
      } as any);
      res.status(201).json({ success: true, data: { sprint }, message: 'Sprint created successfully' } as ApiResponse);
      return;
    }
...
export const getSprints = async (req: PMAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (isPmPostgres()) {
      const sprintRepo = getPmRepos().sprint as PgSprintRepository;
      const taskRepo = getPmRepos().task as PgTaskRepository;

      const statusFilter = status ? (status as string).includes(',') ? (status as string).split(',').map(s => s.trim()) : (status as string) : undefined;
      const sprints = await sprintRepo.findByProject(projectId, statusFilter);

      const sprintsWithStats = await Promise.all(
        sprints.map(async (sprint: any) => {
          const sid = sprint._id || sprint.id;
          const cats = await taskRepo.countByStatusCategory(sid);
          const totalTasks = Object.values(cats).reduce((s: number, v: any) => s + v, 0);
          const completedTasks = cats['done'] || 0;
          // story points would need a custom query; use 0 as placeholder
          return {
            ...sprint,
            stats: { totalTasks, completedTasks, totalPoints: 0, completedPoints: 0 },
            progress: totalTasks > 0 ? Math.round((completedTasks / totalTasks) * 100) : 0,
          };
        })
      );

      res.status(200).json({ success: true, data: { sprints: sprintsWithStats } } as ApiResponse);
      return;
    }
...
export const getSprint = async (req: PMAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (isPmPostgres()) {
      ...
      const result = await taskRepo.searchTasks({ sprintId, sort: 'columnOrder', sortDir: 'asc' }, 1, 1000);
      ...
      const completedPoints = tasks.filter((t: any) => t.statusCategory === 'done').reduce((sum: number, t: any) => sum + (t.storyPoints || 0), 0);
      res.status(200).json({
        success: true,
        data: { sprint, tasks, stats: { totalTasks, completedTasks, totalPoints, completedPoints } },
      } as ApiResponse);
      return;
    }
...
export const updateSprint = async (req: PMAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // ── PostgreSQL path ──
    if (isPmPostgres()) {
      const repo = getPmRepos().sprint as PgSprintRepository;
      let sprint = await repo.findById(sprintId);
      if (!sprint) {
        res.status(404).json({ success: false, error: 'Sprint not found' } as ApiResponse);
        return;
      }
      ...
      if (capacity?.planned !== undefined) updates.capacityPlanned = capacity.planned;
      if (capacity?.committed !== undefined) updates.capacityCommitted = capacity.committed;
      if (capacity?.available !== undefined) updates.capacityAvailable = capacity.available;
      sprint = await repo.updateById(sprintId, updates as any);
      res.status(200).json({ success: true, data: { sprint }, message: 'Sprint updated successfully' } as ApiResponse);
      return;
    }
...
export const startSprint = async (req: PMAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Notify all project members that sprint started
    notifyProjectMembers(sprint.projectId, {
      type: 'task',
      title: `Sprint started: ${sprint.name}`,
      message: `Sprint "${sprint.name}" has started`,
      actionUrl: `/projects/${sprint.projectId}/board`,
    }, userId).catch(() => {});
...
export const completeSprint = async (req: PMAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Notify all project members that sprint completed
    notifyProjectMembers(sprint.projectId, {
      type: 'task',
      title: `Sprint completed: ${sprint.name}`,
      message: `Sprint "${sprint.name}" completed — ${completedTasks.length} tasks done (${completedPoints} pts)`,
      actionUrl: `/projects/${sprint.projectId}/sprints`,
    }).catch(() => {});
...
export const deleteSprint = async (req: PMAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (isPmPostgres()) {
      ...
      if (sprint.status === 'active') {
        res.status(400).json({ success: false, error: 'Cannot delete an active sprint' } as ApiResponse);
        return;
      }
      // Unset sprint from tasks
      const sprintTasks = await taskRepo.findBySprint(sprintId);
      if (sprintTasks.length > 0) {
        await taskRepo.bulkSetSprint(sprintTasks.map((t: any) => t._id || t.id), null);
      }
      await sprintRepo.deleteById(sprintId);
      res.status(200).json({ success: true, message: 'Sprint deleted successfully' } as ApiResponse);
      return;
    }
...
export const getBacklog = async (req: PMAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (isPmPostgres()) {
      const taskRepo = getPmRepos().task as PgTaskRepository;
      // Search tasks with no sprintId
      const result = await taskRepo.searchTasks(
        { projectId, sprintId: null, sort: 'columnOrder', sortDir: 'asc' },
        1,
        1000,
      );
      res.status(200).json({ success: true, data: { tasks: result.data } } as ApiResponse);
      return;
    }
