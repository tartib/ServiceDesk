...
import PMBoard, { IBoardColumn } from '../models/Board';
import { PMStatusCategory } from '../models/Task';
...
export const getWorkflowByProject = async (req: PMAuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const organizationId = req.user?.organizationId || project.organizationId;
    ...
    if (!workflow) {
      const wfDef = defaultWorkflows[project.methodology.code];
      if (!wfDef) {
        res.status(404).json({
          success: false,
      ...
      workflow = await Workflow.create({
        ...
        projectId,
        isDefault: false,
        createdBy: req.user?.id || project.createdBy,
      });
      logger.info(`[workflow] auto-created workflow for project ${projectId} (${project.methodology.code})`);
    }
